...
export default function FloatingAuthModal({toggleToSwithUserButton}) {
  ...
  return (
    <div className="flex   ">
      {/* 打开模态框的按钮 */}
      <button
        className="btn bg-transparent btn-ghost w-18 flex items-center justify-center space-x-2"
  ...
  {isOpen && (
    ...
    >
      {/* 模态框内容 */}
      <motion.div
        className="relative flex flex-col sm:flex-row w-full max-w-sm sm:max-w-[900px] h-auto sm:h-[500px] bg-white rounded-lg shadow-lg overflow-hidden"
      ...
      >
        ...
        {/* 左侧动态部分 */}
        <motion.div
          className={`w-full sm:w-1/2 h-[200px] sm:h-full bg-gradient-to-t ${
            isRegister ? "from-blue-100 to-blue-300" : "from-pink-100 to-pink-300"
        ...
        {/* 右侧动态表单部分 */}
        <motion.div
          className="w-full sm:w-1/2 h-auto sm:h-full bg-gray-50 flex flex-col items-center justify-center p-4"
          initial={{ x: isRegister ? "-100%" : "0%", opacity: 0 }}
          animate={{ x: isRegister ? "-100%" : "0%", opacity: 1 }}
          exit={{ x: isRegister ? "-100%" : "100%", opacity: 0 }}
          ...
          <div className="w-full sm:w-4/5 flex flex-col items-center">
            {isRegister ? (
              <Register
                ...
                setAlert={setAlert}
                toggleToClose={toggleToClose}
              />
            ) : (
              <SignIn
                toggleToRegister={toggleToRegister}
                toggleToSwithUserButton={toggleToSwithUserButton}
                setAlert={setAlert}
              />
            )}
          </div>
